<div class="front_page_section front_page_section_blog<?php
	$briny_scheme = briny_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $briny_scheme ) && ! briny_is_inherit( $briny_scheme ) ) {
		echo ' scheme_' . esc_attr( $briny_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( briny_get_theme_option( 'front_page_blog_paddings' ) );
?>"
		<?php
		$briny_css      = '';
		$briny_bg_image = briny_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$briny_anchor_icon = briny_get_theme_option( 'front_page_blog_anchor_icon' );
	$briny_anchor_text = briny_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( briny_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$briny_css      = '';
			$briny_bg_mask  = briny_get_theme_option( 'front_page_blog_bg_mask' );
			$briny_bg_color_type = briny_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $briny_bg_color_type ) {
				$briny_bg_color = briny_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$briny_caption = briny_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $briny_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $briny_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $briny_caption, 'briny_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$briny_description = briny_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $briny_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $briny_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $briny_description ), 'briny_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
				...
				if ( ! briny_exists_trx_addons() ) {
					...
					briny_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
				}
			}
			?>
			</div>
