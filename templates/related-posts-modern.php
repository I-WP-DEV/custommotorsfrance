	...
	briny_show_post_featured(
		array(
			...
			'post_info'     => '<div class="post_header entry-header">'
									. '<div class="post_categories">' . wp_kses( briny_get_post_categories( '' ), 'briny_kses_content' ) . '</div>'
									. '<h6 class="post_title entry-title"><a href="' . esc_url( $briny_link ) . '">' . wp_kses_data( get_the_title() ) . '</a></h6>'
									. ( in_array( get_post_type(), array( 'post', 'attachment' ) )
											? '<div class="post_meta"><a href="' . esc_url( $briny_link ) . '" class="post_meta_item post_date">' . wp_kses( briny_get_date(), 'briny_kses_content' ) . '</a></div>'
											: '' )
								. '</div>',
		)
	);
