...
$briny_header_id = briny_get_custom_header_id();
$briny_header_meta = get_post_meta( $briny_header_id, 'trx_addons_options', true );
if ( ! empty( $briny_header_meta['margin'] ) ) {
	briny_add_inline_css( sprintf( '.page_content_wrap{padding-top:%s}', esc_attr( briny_prepare_css_value( $briny_header_meta['margin'] ) ) ) );
}

?><header class="top_panel top_panel_custom top_panel_custom_<?php echo esc_attr( $briny_header_id ); ?> top_panel_custom_<?php echo esc_attr(sanitize_title(get_the_title($briny_header_id))); ?>
				<?php
				echo ! empty( $briny_header_image ) || ! empty( $briny_header_video )
					? ' with_bg_image'
					: ' without_bg_image';
...
">
	...
	do_action( 'briny_action_show_layout', $briny_header_id );
