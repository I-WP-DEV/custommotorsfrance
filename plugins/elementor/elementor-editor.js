	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="briny_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.briny_post_editor' );
					if ( link.length > 0 ) {
						briny_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.briny_post_editor', function(e) {
					briny_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('briny_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function briny_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
