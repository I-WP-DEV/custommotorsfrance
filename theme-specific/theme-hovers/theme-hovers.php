...
if ( ! function_exists( 'briny_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'briny_hovers_theme_setup3', 3 );
	function briny_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		briny_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'briny' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'briny' ) ),
					'std'     => 'slide_left',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'briny' ),
						'slide_left'   => esc_html__( 'Slide from Left', 'briny' ),
						'slide_right'  => esc_html__( 'Slide from Right', 'briny' ),
						'slide_top'    => esc_html__( 'Slide from Top', 'briny' ),
						'slide_bottom' => esc_html__( 'Slide from Bottom', 'briny' ),
			
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'briny' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'briny' ) ),
					'std'      => 'dots',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'briny' ),
					),
					'options'  => briny_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'briny_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'briny_hovers_theme_setup9', 9 );
	function briny_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'briny_hovers_frontend_scripts', 1010 );
		add_action( 'wp_enqueue_scripts', 'briny_hovers_frontend_styles', 1100 );
		add_action( 'wp_enqueue_scripts', 'briny_hovers_responsive_styles', 2000 );
		add_filter( 'briny_filter_localize_script', 'briny_hovers_localize_script' );
		add_filter( 'briny_filter_merge_scripts', 'briny_hovers_merge_scripts' );
		add_filter( 'briny_filter_merge_styles', 'briny_hovers_merge_styles' );
		add_filter( 'briny_filter_merge_styles_responsive', 'briny_hovers_merge_styles_responsive' );
		add_filter( 'briny_filter_get_css', 'briny_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'briny_hovers_frontend_scripts' ) ) {
	
	function briny_hovers_frontend_scripts() {
		if ( briny_is_on( briny_get_theme_option( 'debug_mode' ) ) ) {
			$briny_url = briny_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $briny_url ) {
				wp_enqueue_script( 'briny-hovers', $briny_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'briny_hovers_frontend_styles' ) ) {
	
	function briny_hovers_frontend_styles() {
		if ( briny_is_on( briny_get_theme_option( 'debug_mode' ) ) ) {
			$briny_url = briny_get_file_url( 'theme-specific/theme-hovers/theme-hovers.css' );
			if ( '' != $briny_url ) {
				wp_enqueue_style( 'briny-hovers', $briny_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'briny_hovers_responsive_styles' ) ) {
	
	function briny_hovers_responsive_styles() {
		if ( briny_is_on( briny_get_theme_option( 'debug_mode' ) ) ) {
			$briny_url = briny_get_file_url( 'theme-specific/theme-hovers/theme-hovers-responsive.css' );
			if ( '' != $briny_url ) {
				wp_enqueue_style( 'briny-hovers-responsive', $briny_url, array(), null );
			}
		}
	}
}
...
if ( ! function_exists( 'briny_hovers_merge_styles' ) ) {
	
	function briny_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'briny_hovers_merge_styles_responsive' ) ) {
	
	function briny_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'briny_hovers_localize_script' ) ) {
	
	function briny_hovers_localize_script( $arr ) {
		$arr['button_hover'] = briny_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'briny_hovers_merge_scripts' ) ) {
	
	function briny_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'briny_hovers_add_icons' ) ) {
	function briny_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php briny_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'briny' ) : esc_html__( 'Buy now', 'briny' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				?>
				<a href="<?php echo esc_url( is_object( $args['cat'] ) ? get_term_link( $args['cat']->slug, 'product_cat' ) : esc_url( get_permalink() ) ); ?>" aria-hidden="true" class="shop_link button icon-link">
				<?php
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'briny' );
					} else {
						esc_html_e( 'Details', 'briny' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php briny_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : esc_attr($no_link); ?>" <?php briny_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php briny_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php briny_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'briny_hovers_get_css' ) ) {
	
	function briny_hovers_get_css( $css, $args ) {
...
.sc_button_hover_style_link3.sc_button_hover_slide_left {	background: linear-gradient(to right,	{$colors['alter_hover3']} 50%, {$colors['alter_link3']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['alter_link3']} !important; }
.sc_button_hover_style_link3.sc_button_hover_slide_right {  background: linear-gradient(to left,	{$colors['alter_hover3']} 50%, {$colors['alter_link3']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['alter_link3']} !important; }
.sc_button_hover_style_link3.sc_button_hover_slide_top {	background: linear-gradient(to bottom,	{$colors['alter_hover3']} 50%, {$colors['alter_link3']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['alter_link3']} !important; }
.sc_button_hover_style_link3.sc_button_hover_slide_bottom {	background: linear-gradient(to top,		{$colors['alter_hover3']} 50%, {$colors['alter_link3']} 50%) no-repeat scroll right top / 100% 210% {$colors['alter_link3']} !important; }

.sc_button_hover_style_dark.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['alter_link2']} 50%, {$colors['alter_bd_hover']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['inverse_text']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_right {		background: linear-gradient(to left,	{$colors['alter_link2']} 50%, {$colors['alter_bd_hover']} 50%) no-repeat scroll left bottom / 210% 100% {$colors['text_dark']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_top {			background: linear-gradient(to bottom,	{$colors['alter_link2']} 50%, {$colors['alter_bd_hover']} 50%) no-repeat scroll right bottom / 100% 210% {$colors['text_dark']} !important; }
.sc_button_hover_style_dark.sc_button_hover_slide_bottom {		background: linear-gradient(to top,		{$colors['alter_link2']} 50%, {$colors['alter_bd_hover']} 50%) no-repeat scroll right top / 100% 210% {$colors['text_dark']} !important; }
...
.sc_button_hover_style_alterbd.sc_button_hover_slide_left {		background: linear-gradient(to right,	{$colors['text_hover']} 50%, {$colors['text_link']} 50%) no-repeat scroll right bottom / 210% 100% {$colors['text_link']} !important; }
