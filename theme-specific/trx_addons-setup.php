...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'briny_trx_addons_theme_specific_components' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'briny_trx_addons_theme_specific_components' );
	function briny_trx_addons_theme_specific_components( $enable = false ) {
		return false;
	}
}

if ( ! function_exists( 'briny_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'briny_trx_addons_theme_specific_setup1', 1 );
	function briny_trx_addons_theme_specific_setup1() {
		if ( briny_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'briny_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'briny_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'briny_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'briny_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'briny_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'briny_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'briny_trx_addons_widgets_list' );
		}
	}
}

if ( ! function_exists( 'briny_trx_addons_theme_specific_setup' ) ) {
	add_action( 'after_setup_theme', 'briny_trx_addons_theme_specific_setup' );
	function briny_trx_addons_theme_specific_setup() {
		if ( briny_exists_trx_addons() ) {
			if ( ! is_admin() ) {
				add_action( 'briny_action_before_post_meta', 'briny_trx_addons_action_before_post_meta' );
			}
		}
	}
}
...
// CV
if ( ! function_exists( 'briny_trx_addons_cv_enable' ) ) {
	
	function briny_trx_addons_cv_enable( $enable = false ) {
		// To do: return false if theme not use CV functionality
		return BRINY_THEME_FREE
					? false     // Free version
					: true;     // Pro version
	}
}
...
// Demo mode
if ( ! function_exists( 'briny_trx_addons_demo_enable' ) ) {
	
	function briny_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'briny_trx_addons_edd_themes_market_enable' ) ) {
	
	function briny_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'briny_trx_addons_api_list' ) ) {
	
	function briny_trx_addons_api_list( $list = array() ) {
...
// CPT
if ( ! function_exists( 'briny_trx_addons_cpt_list' ) ) {
	
	function briny_trx_addons_cpt_list( $list = array() ) {
...
// Shortcodes
if ( ! function_exists( 'briny_trx_addons_sc_list' ) ) {
	
	function briny_trx_addons_sc_list( $list = array() ) {
...
// Widgets
if ( ! function_exists( 'briny_trx_addons_widgets_list' ) ) {
	
	function briny_trx_addons_widgets_list( $list = array() ) {
        ...
        foreach ( $list as $k => $v ) {
            if ( $k == 'recent_news' ) {
                $list[ $k ]['layouts_sc']['news-extra'] = esc_html__('Extra','briny');
...
if ( ! function_exists( 'briny_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'briny_trx_addons_menu_cache' );
	function briny_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'briny_trx_addons_localize_script' ) ) {
	...
	function briny_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = briny_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'briny_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'briny_trx_addons_sc_type', 10, 2 );
	function briny_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = briny_array_merge( $list, briny_get_list_blog_styles( false, 'sc' ) );
		}
        if ( 'trx_sc_title' == $sc ) {
...
if ( ! function_exists( 'briny_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'briny_trx_addons_sc_prepare_atts', 10, 2 );
	function briny_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = briny_get_list_blog_styles( false, 'sc' );
			if ( isset( $list[ $atts['type'] ] ) ) {
				$custom_type = '';
				if ( strpos( $atts['type'], 'blog-custom-' ) === 0 ) {
					$blog_id = briny_get_custom_blog_id( $atts['type'] );
...
if ( ! function_exists( 'briny_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'briny_trx_addons_sc_atts', 10, 2 );
	function briny_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'briny_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'briny_trx_addons_sc_map', 10, 2 );
	function briny_trx_addons_sc_map( $params, $sc ) {
		...
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'briny' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'briny' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( briny_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'briny' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'briny' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( briny_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'briny_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'briny_trx_addons_sc_output', 10, 4 );
	function briny_trx_addons_sc_output( $output, $sc, $atts, $content ) {
		...
		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			...
			if ( ! empty( $atts['color_style'] ) && ! briny_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'briny_trx_addons_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes', 'briny_trx_addons_sc_item_link_classes', 10, 3 );
	function briny_trx_addons_sc_item_link_classes( $class, $sc, $atts=array() ) {
		if ( 'sc_button' == $sc ) {
			if ( ! empty( $atts['color_style'] ) && ! briny_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'briny_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'briny_trx_addons_sc_item_title_tag' );
	function briny_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'briny_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'briny_trx_addons_sc_item_button_args', 10, 3 );
	function briny_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'briny_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'briny_trx_addons_slider_title', 10, 2 );
	function briny_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'briny_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'briny_trx_addons_sc_googlemap_styles' );
	function briny_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'briny' );
		return $list;
	}
}

// Show reactions in the single posts
if ( ! function_exists( 'briny_trx_addons_action_before_post_meta' ) ) {
	
	function briny_trx_addons_action_before_post_meta() {
		if ( briny_exists_trx_addons() ) {
...
if ( ! function_exists( 'briny_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'briny_trx_addons_tiny_mce_style_formats' );
	function briny_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'briny' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'briny' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'briny_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'briny_filter_allow_override_header_image', 'briny_trx_addons_allow_override_header_image' );
	function briny_trx_addons_allow_override_header_image( $allow ) {
		return is_single()
				&& (
					briny_is_team_page()
					|| briny_is_cars_page()
					|| briny_is_cars_agents_page()
					|| briny_is_properties_agents_page()
					)
				? false
				: $allow;
	}
...
if ( ! function_exists( 'briny_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'briny_trx_addons_thumb_size', 10, 2 );
	function briny_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
// All other CPT override optiones may be modified in the same method
if ( ! function_exists( 'briny_trx_addons_override_options_fields' ) ) {
	add_filter( 'trx_addons_filter_override_options_fields', 'briny_trx_addons_override_options_fields', 10, 2 );
	function briny_trx_addons_override_options_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'briny' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'briny' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
...
if ( ! function_exists( 'briny_filter_get_list_menu_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_menu_hover', 'briny_filter_get_list_menu_hover' );
    function briny_filter_get_list_menu_hover( $list ) {
        unset( $list['fade_box'] );
...
if ( ! function_exists( 'briny_filter_get_list_input_hover' ) ) {
    add_filter( 'trx_addons_filter_get_list_input_hover', 'briny_filter_get_list_input_hover' );
    function briny_filter_get_list_input_hover( $list ) {
        unset( $list['accent'] );
        unset( $list['path'] );
        unset( $list['jump'] );
