...
if ( ! function_exists( 'briny_need_page_title' ) ) {
	function briny_need_page_title() {
		return ! is_front_page() && apply_filters( 'briny_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'briny_show_layout' ) ) {
	function briny_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'briny_get_logo_image' ) ) {
	function briny_get_logo_image( $type = '' ) {
		$logo_image  = '';
		if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
			$logo_image = briny_get_theme_option( 'custom_logo' );
			if ( empty( $logo_image ) ) {
			...
			$logo_image = briny_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
		}
		$logo_retina = briny_is_on( briny_get_theme_option( 'logo_retina_enabled' ) )
						? briny_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' )
						: '';
		return array(
					'logo'        => ! empty( $logo_image ) ? briny_remove_protocol_from_url( $logo_image, false ) : '',
					'logo_retina' => ! empty( $logo_retina ) ? briny_remove_protocol_from_url( $logo_retina, false ) : ''
				);
	}
}

// Return header video (if set)
if ( ! function_exists( 'briny_get_header_video' ) ) {
	function briny_get_header_video() {
		$video = '';
		if ( apply_filters( 'briny_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( briny_check_theme_option( 'header_video' ) ) {
				$video = briny_get_theme_option( 'header_video' );
...
// Show post banner
if ( ! function_exists( 'briny_show_post_banner' ) ) {
	function briny_show_post_banner( $banner_pos = '') {
		if ( is_singular( 'post' ) && '' !== $banner_pos ){
			$banner_code   = briny_get_theme_option( $banner_pos . '_banner_code' );
			$banner_img    = briny_get_theme_option( $banner_pos . '_banner_img' );
			$banner_height = briny_get_theme_option( $banner_pos . '_banner_height' );
			$banner_class  = !empty( $banner_img )
								? ( 'background' == $banner_pos ? '' : 'banner_with_image ' )
									. briny_add_inline_css_class( 
										'background-image:url(' . esc_url( $banner_img ) . ');'
										. ( ! empty( $banner_height )
											? 'min-height:' . briny_prepare_css_value( $banner_height ) . ';'
											: ''
											)
										)
								: '';
			$banner_link   = briny_get_theme_option( $banner_pos . '_banner_link' );
			if ( ! empty( $banner_code ) || ! empty( $banner_img ) ) {
				...
				briny_show_layout( wp_kses( $banner_code, 'briny_kses_content' ) );
...
if ( ! function_exists( 'briny_show_post_featured_image' ) ) {
	function briny_show_post_featured_image() {
		$seo = briny_is_on( briny_get_theme_option( 'seo_snippets' ) );
		// Featured image
		if ( briny_is_off( briny_get_theme_option( 'hide_featured_on_single' ) )
					&& ! briny_sc_layouts_showed( 'featured' )
					&& strpos( briny_get_post_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'briny_action_before_post_featured' );
			briny_show_post_featured(
				array(
					'singular' => true,
					'thumb_bg' => briny_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth'
				)
			);
...
if ( ! function_exists( 'briny_show_post_title_and_meta' ) ) {
	function briny_show_post_title_and_meta() {

		$seo = briny_is_on( briny_get_theme_option( 'seo_snippets' ) );

		// Title and post meta
		if ( ( ! briny_sc_layouts_showed( 'title' ) || ! briny_sc_layouts_showed( 'postmeta' ) ) && ! in_array( get_post_format(), array( 'link', 'aside', 'status', 'quote' ) ) ) {
			do_action( 'briny_action_before_post_title' );
			$need_content_wrap = briny_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth' && briny_get_theme_option( 'post_header_position' ) !== 'on_thumb';
			?>
			<div class="post_header post_header_single entry-header">
				...
				// Post meta
				$meta_components = briny_array_get_keys_by_value( briny_get_theme_option( 'meta_parts' ) );
				...
				if ( $show_categories && ! briny_sc_layouts_showed( 'postmeta' ) && briny_is_on( briny_get_theme_option( 'show_post_meta' ) ) ) {
					briny_show_post_meta(
						apply_filters(
							'briny_filter_post_meta_args',
							array(
				...
				// Post title
				if ( ! briny_sc_layouts_showed( 'title' ) ) {
					the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
				}
				// Post subtitle
				$post_subtitle = briny_get_theme_option( 'post_subtitle' );
				if ( ! empty( $post_subtitle ) ) {
					?>
					<div class="post_subtitle">
						<?php briny_show_layout( $post_subtitle ); ?>
					</div>
					<?php
				}
				// Post meta
				if ( ! briny_sc_layouts_showed( 'postmeta' ) && briny_is_on( briny_get_theme_option( 'show_post_meta' ) ) ) {
					briny_show_post_meta(
						apply_filters(
							'briny_filter_post_meta_args',
							array(
			...
			<?php
			do_action( 'briny_action_after_post_title' );
		}
	}
}
...
if ( ! function_exists( 'briny_show_post_content' ) ) {
	function briny_show_post_content( $args = array(), $otag='', $ctag='' ) {
		$plain = true;
		...
		if ( has_excerpt() ) {
			...
			briny_show_layout( briny_filter_post_content( get_the_content('') ) );
			...
			$quote = briny_get_tag( briny_filter_post_content( get_the_content() ), '<blockquote', '</blockquote>' );
			if ( ! empty( $quote ) ) {
				briny_show_layout( ( $quote ) );
			...
			if ( $plain ) {
				$len = ! empty( $args['hide_excerpt'] )
							...
							: ( ! empty( $args['excerpt_length'] )
								...
								: briny_get_theme_option( 'excerpt_length' )
								);
				$output = briny_excerpt( $output, $len );
			}
		}
		briny_show_layout( $output, $otag, $ctag);
	}
}


// Show post link 'Read more' in the blog posts
if ( ! function_exists( 'briny_show_post_more_link' ) ) {
	function briny_show_post_more_link( $args = array(), $otag='', $ctag='' ) {
		briny_show_layout(
			'<a class="more-link" href="' . esc_url( get_permalink() ) . '">'
				. ( ! empty( $args['more_text'] )
						? esc_html( $args['more_text'] )
						: esc_html__( 'Read more', 'briny' )
						)
			. '</a>',
			$otag,
			$ctag
...
if ( ! function_exists( 'briny_show_post_meta' ) ) {

	function briny_show_post_meta( $args = array() ) {
		if ( is_single() && briny_is_off( briny_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta<?php echo ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : ''; ?>">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ', ' ) : apply_filters( 'briny_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						briny_show_layout( $cats, '<span class="post_meta_item post_categories">', '</span> ');
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'briny_filter_get_post_date', briny_get_date( '', ! empty( $args['date_format'] ) ? $args['date_format'] : '' ) );
					if ( ! empty( $dt ) ) {
						briny_show_layout(
							$dt,
					...
					if ( $author_id > 0 ) {
						...
						$author_avatar = get_avatar( get_the_author_meta( 'user_email', $author_id ), 32 * briny_get_retina_multiplier() );
						echo '<a class="post_meta_item post_author" rel="author" href="' . esc_url( $author_link ) . '">'
								. ( briny_get_theme_setting( 'show_author_avatar' )
					...
					if ( !is_singular() || have_comments() || comments_open() ) {
						...
						echo '<a href="' . esc_url( get_comments_link() ) . '" class="post_meta_item post_meta_comments icon-comment-light">'
								...
								. '<span class="post_meta_label">' . esc_html( ($post_comments === 1) ? esc_html__('Comment', 'briny') :  esc_html__('Comments', 'briny') ) . '</span>'
							. '</a> ';
					}
					...
					if ( function_exists( 'trx_addons_get_post_views' ) ) {
						...
						echo ( is_singular()
							...
							. ' class="post_meta_item post_meta_views trx_addons_icon-eye">'
								...
								. '<span class="post_meta_label">' . esc_html( ($post_views === 1) ? esc_html__('View', 'briny') :  esc_html__('Views', 'briny') ) . '</span>'
							. ( is_singular()
								? '</span> '
								: '</a> '
								);
				...
				// Likes (Emotions)
					...
					if ( function_exists( 'trx_addons_get_post_likes' ) ) {
						...
						$liked = isset( $_COOKIE['trx_addons_likes'] ) ? briny_get_value_gpc('trx_addons_likes') : '';
						...
						echo ( true == $emotions_allowed
								...
								: '<a href="#"'
									...
									. ' title="' . ( ! empty( $allow ) ? esc_attr__( 'Like', 'briny') : esc_attr__( 'Dislike', 'briny' ) ) . '"'
									. ' data-postid="' . esc_attr( get_the_ID() ) . '"'
									. ' data-likes="' . esc_attr( $post_likes ) . '"'
									. ' data-title-like="' . esc_attr__( 'Like', 'briny') . '"'
									. ' data-title-dislike="' . esc_attr__( 'Dislike', 'briny' ) . '"'
									. '>'
								)
									. '<span class="post_meta_number">' . esc_html($post_likes) . '</span>'
									. '<span class="post_meta_label">'
										. ( true == $emotions_allowed
											? (esc_html( ($post_likes === 1) ? esc_html__('Reaction', 'briny') :  esc_html__('Reactions', 'briny') ))
											: (esc_html( ($post_likes === 1) ? esc_html__('Like', 'briny') :  esc_html__('Likes', 'briny') ))
											)
									. '</span>'
								. '</a> ';
					}
					...
					// Socials share
					briny_show_share_links(
						array(
							'type'    => $args['share_type'],
							'caption' => 'drop' == $args['share_type'] ? esc_html__( 'Share', 'briny' ) : '',
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'briny' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );

				} else {
					// Custom counter
					do_action( 'briny_action_show_post_meta', $comp, get_the_ID(), $args );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'briny_show_post_featured' ) ) {
	function briny_show_post_featured( $args = array() ) {

		$args = array_merge(
			array(
				'hover'         => briny_get_theme_option( 'image_hover' ), // Hover effect
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: briny_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? briny_get_no_image( '', true ) : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = briny_get_post_image();
				if ( ! empty( $image ) ) {
					$image = briny_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . briny_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
		...
		if ( ! empty( $args['singular'] ) ) {
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						briny_is_on( briny_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( briny_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				$output .= '>';
				briny_show_layout( $output );
				if ( has_post_thumbnail() && $args['seo'] ) {
					$briny_attr = briny_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $briny_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $briny_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = briny_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						briny_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) ) ) {
					$post_content = briny_get_post_content();
					$post_content_parsed = $post_content;

					if ( 'video' == $post_format ) {
						$video = briny_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = briny_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							// Only get video from the content if a playlist isn't present.
							$post_content_parsed = briny_filter_post_content( $post_content );
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = briny_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = briny_make_video_autoplay( $video );
								?>
						...
						$audio = briny_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = briny_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = briny_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								if ( strpos( $audio, '<audio' ) !== false ) {
									$media_title  = briny_get_tag_attrib( $audio, '<audio>', 'data-caption' );
									$media_author = briny_get_tag_attrib( $audio, '<audio>', 'data-author' );
								}
								if ( empty( $media_author) &&  empty( $media_title) ) {
									$media = urldecode( briny_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
									if ( ! empty( $media ) ) {
										...
										// Parse tag params
										$media_title  = strip_tags( briny_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );
										$media_author = strip_tags( briny_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );

									}
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php briny_show_layout( $media_title ); ?></h5>
									<?php
								}
                                if ( ! empty( $media_author ) ) {
                                    ?>
                                    <div class="post_audio_author"><?php briny_show_layout( $media_author ); ?></div>
                                    <?php
								}
								// Display audio
								briny_show_layout( $audio );
								?>
							</div>
							<?php
						}
...
if ( ! function_exists( 'briny_get_no_image' ) ) {
	function briny_get_no_image( $no_image = '', $need = false ) {
		static $no_image_url = '';
		$img = briny_get_theme_option( 'no_image' );
		if ( empty( $img ) && ( $need || briny_get_theme_setting( 'allow_no_image' ) ) ) {
			if ( empty( $no_image_url ) ) {
				$no_image_url = briny_get_file_url( 'images/no-image.jpg' );
			}
			$img = $no_image_url;
		}
		if ( ! empty( $img ) ) {
...
if ( ! function_exists( 'briny_add_bg_in_post_nav' ) ) {
	function briny_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), briny_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
			if ( ! empty( $img ) ) {
				...
				$css .= '.post-navigation .nav-previous a .nav-arrow:after { top: 0; opacity: 1; }';
			}
		}
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), briny_get_thumb_size( 'med' ) );
				$img = $img[0];
			} else {
				$img = $noimg;
			}
			if ( ! empty( $img ) ) {
				...
				$css .= '.post-navigation .nav-next a .nav-arrow:after { top: 0; opacity: 1; }';
			}
		}

		wp_add_inline_style( 'briny-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'briny_show_related_posts' ) ) {
	function briny_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : briny_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) briny_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			$related_position = briny_get_theme_option( 'related_position' );
			$related_style = briny_get_theme_option( 'related_style' );
			...
			<section class="related_wrap related_position_<?php echo esc_attr( $related_position ); ?> related_style_<?php echo esc_attr( $related_style ); ?>">
				<<?php echo esc_html( $related_tag ); ?> class="section_title related_wrap_title"><?php
					if ( ! empty( $title ) ) {
						...
						esc_html_e( 'You May Also Like', 'briny' );
					}
				...
				if ( $args['slider'] ) {
					...
					$slider_args['slides_space']      = briny_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = briny_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = briny_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'briny_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					briny_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						get_template_part( apply_filters( 'briny_filter_get_template_part', 'templates/related-posts', $style ), $style );
						?></div><?php
					}
				?></div><?php		
				if ( $args['slider'] ) {
					briny_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'briny_show_related_posts_callback' ) ) {
	add_action( 'briny_action_related_posts', 'briny_show_related_posts_callback' );
	function briny_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'briny_filter_show_related_posts', false ) ) {
			$briny_related_posts   = (int) briny_get_theme_option( 'related_posts' );
			$briny_related_columns = (int) briny_get_theme_option( 'related_columns' );
			$briny_related_style   = briny_get_theme_option( 'related_style' );
			if ( (int) briny_get_theme_option( 'show_related_posts' ) && $briny_related_posts > 0 ) {
				briny_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $briny_related_posts ) ),
						'columns'        => max( 1, min( 6, $briny_related_posts, $briny_related_columns ) ),
					),
					$briny_related_style
				);
			}
...
if ( ! function_exists( 'briny_show_portfolio_posts' ) ) {
	function briny_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? briny_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = briny_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = briny_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$briny_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the briny_ajax_get_posts action
if ( ! function_exists( 'briny_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_briny_ajax_get_posts', 'briny_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_briny_ajax_get_posts', 'briny_ajax_get_posts_callback' );
	function briny_ajax_get_posts_callback() {
		...
		if ( (int) $id > 0 ) {
			...
			briny_storage_set( 'blog_mode', 'blog' );
			briny_storage_set( 'options_meta', get_post_meta( $id, 'briny_options', true ) );
		}

		$response = array(
			'error' => '',
			'data'  => briny_show_portfolio_posts(
				array(
					'cat'        => intval( wp_unslash( briny_get_value_gpc('cat') ) ),
					'parent_cat' => intval( wp_unslash( briny_get_value_gpc('parent_cat') ) ),
					'page'       => intval( wp_unslash( briny_get_value_gpc('page') ) ),
					'post_type'  => trim( wp_unslash( briny_get_value_gpc('post_type') ) ),
					'taxonomy'   => trim( wp_unslash( briny_get_value_gpc('taxonomy') ) ),
					'blog_style' => trim( wp_unslash( briny_get_value_gpc('blog_style') ) ),
					'echo'       => false,
				)
			),
		);
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'briny' );
		}
...
// Show pagination
if ( ! function_exists( 'briny_show_pagination' ) ) {
	function briny_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = briny_get_theme_option( 'blog_pagination' );
		if ( 'pages' == $pagination ) {
			the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'briny' ),
					'next_text'          => esc_html__( '>', 'briny' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'briny' ) . ' </span>',
				)
			);
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e( 'SHOW MORE STORIES', 'briny' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'briny' ) : esc_html__( 'Newest posts', 'briny' ) ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'briny' ) : esc_html__( 'Older posts', 'briny' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'briny_single_comments_field' ) ) {
	function briny_single_comments_field( $args ) {
		...
		if ( 'text' == $args['field_type'] ) {
			...
			$html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'briny_kses_content' ) . '</label>';
		} else {
		...
		if ( 'default' != $args['form_style'] && in_array( $args['field_type'], array( 'text', 'textarea' ) ) ) {
			$html .= '<span class="sc_form_field_hover">'
						...
						. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'], 'briny_kses_content' ) . '</span>'
					. '</span>';
		}
		$html .= '</span></div>';
		return $html;
